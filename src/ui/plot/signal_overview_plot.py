    ...
    def update_plot(self):
        ...
        for i, ch in enumerate(ch_indices):
            ...
            if trace.shape[0] != time.shape[0]:
                 ...
                 valid_ch_indices_plot_order.pop()
                 continue
            ...
            is_sel = global_state.get_channel_status(ch)
        ...
        if num_plotted_channels > 0:
            self.ax.set_ylim(-0.5, num_plotted_channels - 0.5)

            yticks_pos = np.arange(num_plotted_channels)
            yticks_labels = [str(ch + 1) for ch in valid_ch_indices_plot_order]
            self.ax.set_yticks(yticks_pos)
            self.ax.set_yticklabels(yticks_labels, fontsize=8)
